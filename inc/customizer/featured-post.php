<?php
/**
 * Adds wavypixel_Featured_Posts_Widget widget.
 */
class wavypixel_Featured_Posts_Widget extends WP_Widget {
	...
	function __construct() {
		parent::__construct(
			'wavypixel_featured_posts_widget', // Base ID
			esc_html__( 'Featured Posts', 'wavypixel' ), // Name
			array( 'description' => esc_html__( 'A Widget to display featured posts', 'wavypixel' ) ) // Args
		);
	}
	...
	public function form( $instance ) {
		$title            = ! empty( $instance['title'] ) ? $instance['title'] : esc_html__( 'Featured Posts', 'wavypixel' );
		$number_of_posts  = ! empty( $instance['number_of_posts'] ) ? absint( $instance['number_of_posts'] ) : 4;
		$order            = ! empty( $instance['order'] ) ? $instance['order'] : 'DESC';
		$category         = ! empty( $instance['category'] ) ? $instance['category'] : '';
		?>
		...
		<p>
			...
			<select class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'order' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'order' ) ); ?>">
				<option value="DESC" <?php selected( $order, 'DESC' ); ?>><?php esc_html_e( 'Descending', 'wavypixel' ); ?></option>
				<option value="ASC" <?php selected( $order, 'ASC' ); ?>><?php esc_html_e( 'Ascending', 'wavypixel' ); ?></option>
			</select>
		</p>
		<p>
...
// register wavypixel_Featured_Posts_Widget widget
function wavypixel_register_featured_posts_widget() {
	register_widget( 'wavypixel_Featured_Posts_Widget' );
}
add_action( 'widgets_init', 'wavypixel_register_featured_posts_widget' );
